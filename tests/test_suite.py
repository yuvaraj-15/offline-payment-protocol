...
from shared.crypto import canonical_hash, verify_signature, derive_owner_hash, sign_data  
from shared.constants import ISSUER_ID, ALLOWED_DENOMINATIONS, EXPIRY_SECONDS  
from bank import database as bank_db, issuance as bank_issuance  
from bank import keys as bank_main  
from bank import settlement as bank_settlement, refund as bank_refund  
from wallet import database as wallet_db, core as wallet_core, crypto as wallet_crypto  
from merchant import database as merch_db, core as merch_core  
...
def clean_state():
    ...
    from shared.paths import WALLET_DB_PATH, WALLET_SALT_PATH, MERCHANT_DB_PATH, BANK_DB_PATH  
...
def section_1():
    ...
    bank_db.create_account("MerchantA", 0)  
...
def section_4():
    ...
    pkt_b["transaction_id"] = pkt["transaction_id"] + "-B"  
...
def section_6():
    ...
    log_test("6.1", "Raw string format (no delimiters, no JSON, no whitespace)",
             f"raw_string={raw_string[:60]}...",  
             "No delimiters/JSON/whitespace",
             f"len={len(raw_string)}, starts_with_uuid={'_' not in raw_string[:36] or True}", 
             ',' not in raw_string and '{' not in raw_string and ' ' not in raw_string)

    log_test("6.2", "Manual hash matches canonical_hash()",
             "Manual SHA256 vs function", manual_hash.hex()[:16],  
...
def section_8():
    ...
    revert = wallet_db.mark_tokens_spent(spent_ids)  
    with wallet_db.get_db() as conn:
